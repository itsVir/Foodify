...
import FoodForm from "../AdminMenuItem/FoodItemForm";
...
const AdminMenu = () => {
  ...
  //   try {
  //     const response = await fetch("http://127.0.0.1:8000/food/menu/create/", {
  //       method: "POST",
  //       headers: {
  //         Authorization: `Bearer ${accessToken}`,
...
console.log(response);
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/food/menu/${id}/`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      ...
      setFoodItems(foodItems.filter((item) => item.MenuID !== id));
    } catch (error) {
    ...
    <div className="Admin_menu">
      ...
      <div className="table_back">
        <table className="food-table">
          ...
          <tbody>
            {foodItems
              .filter((foodItem) => foodItem.CategoryName === "Home")
              .map((foodItem, index) => (
